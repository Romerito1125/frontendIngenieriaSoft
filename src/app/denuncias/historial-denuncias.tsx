...
export function HistorialDenuncias({
  userId,
  onCreateClick,
}: HistorialDenunciasProps) {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      setDenuncias((prev) =>
        prev.filter((denuncia) => denuncia.iddenuncia !== id)
      );
      toast.success("La denuncia ha sido eliminada correctamente");
    } catch (err: any) {
      console.error("Error al eliminar denuncia:", err);
      toast.error(
        "No se pudo eliminar la denuncia. Por favor, intenta nuevamente."
      );
    } finally {
      setDeletingId(null);
    }
  ...
  if (error) {
    return (
      ...
      >
        <Alert
          variant="destructive"
          className="my-6 border-l-4 border-l-red-600 shadow-md"
        >
    ...
    return (
      ...
      >
        ...
        <p className="text-gray-500 mb-8 max-w-md mx-auto">
          Aún no has realizado ninguna denuncia. Puedes crear una nueva para
    ...
    >
      ...
      {searchTerm && (
        <div className="mb-4 text-sm text-gray-500">
          Se encontraron {filteredDenuncias.length} resultados para "
          {searchTerm}"
        </div>
      )}
        ...
        {filteredDenuncias.length === 0 && searchTerm ? (
          <div className="text-center py-8 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <p className="mt-2 text-gray-500">
              No hay denuncias que coincidan con "{searchTerm}"
            </p>
            <Button
              onClick={() => setSearchTerm("")}
              variant="outline"
            ...
            >
              <Card className="overflow-hidden border border-gray-200 hover:border-blue-200 transition-all duration-300 hover:shadow-lg group rounded-xl">
                <CardHeader className="bg-blue-50 pb-3 flex flex-row items-center justify-between">
                  ...
                  >
                    {denuncia.estado.charAt(0).toUpperCase() +
                      denuncia.estado.slice(1)}
                  </Badge>
                </CardHeader>
                ...
                <CardContent className="pt-5">
                  <div className="mb-4 flex items-center p-2 bg-gray-50 rounded-lg group-hover:bg-gray-100 transition-colors duration-300">
                    ...
                    <span className="text-sm font-medium text-gray-700">
                      Tipo: {getTypeLabel(denuncia.tipo)}
                    </span>
                  </div>
                    ...
                    <AlertDialogContent className="max-w-md bg-white border border-gray-200 shadow-xl rounded-xl">
                      <AlertDialogHeader className="pb-4">
                        ...
                        <AlertDialogDescription className="text-gray-600 mt-2">
                          ...
                          permanentemente tu denuncia y no podrás recuperarla.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
